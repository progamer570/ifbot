...
export async function sendToCollection(chat, aIOPosterID, link, caption) {
    try {
...
export async function sendToCollectionOng(chat, link, caption) {
    try {
...
export async function sendToCollectionOng2(chat, aIOPoster, links, shareId = "") {
    if (aIOPoster) {
        try {
            const captionText = `\`\`\`\n${links[0].caption}\n\`\`\``;
        ...
        if (shareId) {
            ...
            for (let i = 0; i < links.length; i += chunkSize) {
                ...
                const formattedLinks = chunk
                    .map((item) => `[${escapeMarkdownV2(convertToTinySubscript(processCaption(item.caption.slice(0, 90), "")))}](https://t.me/${env.botUserName}?start=${item.messageId}-ong)`)
                    .join("\n\n");
...
export async function sendToLogGroup(chat, caption) {
    try {
