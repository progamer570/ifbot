...
async function done(ctx) {
    ...
    if (ctx.message) {
        ...
        if (text.toLowerCase() === "done" && !ctx.session.done) {
            ...
            const forwardedMessageIds = await telegram.forwardMessages(env.dbAIOChannelId, ctx.chat?.id, AIODetails.messageIds ? AIODetails.messageIds : [], false, captions);
            try {
                const AIOData = await getAIOdata(AIODetails, forwardedMessageIds);
                let shareId;
                let link;
                const botUsername = ctx.botInfo.username;
                if (ctx.session.isHindi) {
                ...
                try {
                    if (ctx.session.isHindi) {
                        await sendToCollection(env.collectionHindi, AIOData.aIOPosterID, link, AIOData.aIOTitle || "none");
                        try {
