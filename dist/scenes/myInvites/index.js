...
const myInvitePagination = new Scenes.WizardScene("myInvites", Composer.on("message", async (ctx) => {
    if ("text" in ctx.message) {
        ctx.session.page = 0;
        const myInviteUser = await database.getInviteUser(ctx.from.id.toString());
        if (!myInviteUser) {
            ctx.reply(`Invite atleast 1 users to see your invited user list \nyour invite link is: ${generateInviteLink(ctx.from.id.toString(), false)}`);
        }
        if (myInviteUser) {
...
___________________________________`;
            ...
            try {
                ...
                logger.error("Error replying with invite list:", error);
            }
        }
    ...
    if ("data" in ctx.callbackQuery &&
        ...
        const link = generateInviteLink(ctx.session.id, true);
        const totalInvites = ctx.session.totalInvites;
        const inviteData = ctx.session.inviteData;
        logger.debug("Current page and invite data length:", ctx.session.page || 0, ctx.session.inviteData?.length);
        if (inviteData) {
            try {
                ...
                logger.error("Error handling invite pagination callback:", error);
            }
        }
        else {
